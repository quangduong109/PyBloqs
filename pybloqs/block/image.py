...
class PlotlyPlotBlock(BaseBlock):
    ...
    def __init__(self, contents, plotly_kwargs=None, **kwargs):
        ...
        # data_html = data_html.replace("window.PLOTLYENV=window.PLOTLYENV", "if(typeof require !== 'undefined') { require(['plotly'], function (Plotly){ window.PLOTLYENV = window.PLOTLYENV").replace("</script>", "})}}</script>")
