...
class PlotlyPlotBlock(BaseBlock):
    ...
    def __init__(self, contents, plotly_kwargs=None, **kwargs):
        ...
        data_html = data_html.replace("window.PLOTLYENV=window.PLOTLYENV", "document.onreadystatechange = function () { if (document.readyState === 'complete' || document.readyState === 'interactive') { if(typeof require !== 'undefined') { require(['plotly'], function (Plotly){ window.PLOTLYENV = window.PLOTLYENV").replace("</script>", "})}}}</script>")
